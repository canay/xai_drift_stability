...
def validate_design_adequacy(frames: dict[str, pd.DataFrame]):
    """Apply minimum gates before inferential summaries can support claims."""
    thresholds = {
        "minimum_datasets": 2,
        "minimum_seed_clusters_per_dataset": 3,
        "minimum_repeated_draws_per_cell": 2,
    }
    ...
    for tier in TIERS:
        ...
        checks = {
            ...
            "repeated_draws": (
                ...
                thresholds["minimum_repeated_draws_per_cell"]),
...
def analyze(run_dir: Path, reps: int, bootstrap_seed: int, ci_level: float,
    ...
    if not low_present:
        ...
        validation["notes"].append(
            ...
            "repeated-draw minima were not met for claim-ready inference.")
    else:
        status = "complete"
